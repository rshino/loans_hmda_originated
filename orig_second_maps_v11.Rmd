...
param<-paste0('LGST=',stateabb,' YEARS=',year,' CONF BYSELL W_LEI ORD2HOM MINCOUNT=',MINCOUNT)
...
originator<-read.csv(outfile,sep='\t') ## ,colClasses=c("character","numeric"))
maxrows<-min(TOPN,nrow(originator))
for (i in 1:maxrows)
{

lei<-originator[i,'lei']
param<-paste0('W_LEI F_LEI=',lei,' CONF YEARS=',year,' BYCT W_LEI')
