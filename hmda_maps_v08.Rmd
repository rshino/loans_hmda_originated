...
map_tracts<-function(data,state,title,cbsa)
{
  ...
  breaks<-c(0,0.01,0.05,0.20,0.30,1)
  n<-length(breaks)-1
  palette<-"Reds"
  ...
  map_list<-append(map_list,list(
    tract_choropleth(data,state_name=state,title=map_title,num_colors=1) +
      scale_fill_stepsn(colors=RColorBrewer::brewer.pal(n,palette),breaks=breaks)
    ))
    ...
    for(i in 1:nrow(cbsa_list)) # CBSAs
    {
      ...
      if(nrow(md_list)>0) # if there are Metro Divisions, loop through them
      {
        for(j in 1:nrow(md_list)) # plot each Metro Division seperately
        {
          ...
          map_list<-append(map_list,list(
            tract_choropleth(data,state_name=state,title=map_title,num_colors=1,county_zoom=county_list) +
              scale_fill_stepsn(colors=RColorBrewer::brewer.pal(n,palette),breaks=breaks)
...
outfile<-'hmda_strats_v03.tsv'
sqlfile<-"hmda_strats_v03.sql"
...
if(overwrite | !file.exists(outfile))
{
  system(paste0('cat ',sqlfile, ' | cpp -I. -I.. -DYEAR=',year
                ,' | time mysql --login-path=shino loans | tee ',outfile))
}
...
map_title<-paste("Second Homes in",year)
