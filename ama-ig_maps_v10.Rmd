...
map_tracts<-function(data,state,title,cbsa)
{
  ...
  breaks<-c(0,0.015,100)
  #breaks<-breaks+0.0001
   # breaks<-c(0.01,0.0125,0.015,0.0175,0.10,0.25,0.5,1)
...
```{r ama-ig}
map_title<-'Est. AMA-IG'
loans<-read.csv("tract.csv",sep=',')
hist(loans$Average.of.AMA_IG_._UPB,breaks=20,xlim=c(0,.10))

data<-loans[,c("Row.Labels","Average.of.AMA_IG_._UPB")]
names(data)<-c("region","value")
maps<-list()
```
