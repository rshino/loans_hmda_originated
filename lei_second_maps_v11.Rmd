...
leilist<-list()
leilist$NY<-c('549300RUKY2I2HQ2AI44' # polish & slavic nyc ny
,'549300NVMV4W48B8SC18' # watertown watertown ny
,'254900Q6IHGQO7INVC79' # lyons lyons ny
,'549300F7JKU0UTD6KX15' # evans angola NY
)
leilist$NJ<-c(
'5493002N5168AC238149' # oceanfirst toms river nj
,'5493005LKFCLR81TSN28' # columbia fair lawn nj
,'549300US45HOXL30J494' # financial resources bridgewater nj
)

for(stateabb in stateabb_list)
{
state_name<-state.regions[state.regions$abb==stateabb,'region']
state_code<-state.regions[state.regions$abb==stateabb,'fips.numeric']
if(stateabb=='NY')
{leis<-leilist$NY}
if(stateabb=='NJ')
{leis<-leilist$NJ}

#for (lei in leilist[stateabb])

for (lei in leis)
{
  print(overwrite)
param<-paste0('F_LEI=',lei,' F_2HOM CONF YEARS=',year,' BYCT')
  print(param)
...
seller<-hmda_data$seller
map_title<-paste0(seller,'\nSecond Home Orig '
                 ##,sprintf('%2.1f%%',100*originator[i,'second_home_pct'])
                  ,'\n',param)
#data<-hmda_data[,c('census_tract','second_home_pct')]
data<-hmda_data[,c('census_tract','Loans')]
